...
measured activity, a 287-residue wild type. It is there to show the workflow runs end to end inside
one Colab session; its 13 test variants are not a benchmark. Open **ColabSeqDisplay**, set
`Runtime ▸ Change runtime type ▸ T4 GPU`, run the setup cell, change nothing, press train, then
press **Write performance_report.zip** — training is estimated at about a minute on a free T4.
...
`examples/mg8_petases/library.csv` exactly as it appears in the file:

| Column | What it holds | Required | Example row |
|---|---|---|---|
| `p22` … `p279` | One column per mutated site, in the order of the positions you give. Three-letter or one-letter residue codes (a form field says which). | yes | `Y`, `V`, `S`, `I`, `F`, `V`, `S`, `T`, `R`, `T`, `W`, `V`, `V`, `H`, `A`, `G`, `S`, `N`, `T` |
| `count` | Reads supporting the variant. Used only to drop low-count rows via `min_count`. | no | — (this example has none) |
| `activity` | One column per measured condition. Numeric activity, all predicted jointly by one multi-condition head. | yes | `1.57318039171052` |

The names and their number are yours: any number of mutated sites, any number of measured
conditions, and a column you do not name — the example's `variant` labels — is carried through
untouched. Three form fields tie the CSV to the protein — the wild-type sequence (pasted, or the
URL of a FASTA), the 1-based positions of the mutated sites, and the mutation column names in the
same order. In the example: positions 22, 42, 49, 72, 78, 79, 80, 130, 131, 132, 150, 198, 199,
205, 206, 215, 216, 225 and 279 of the 287-residue wild type, whose residues there spell
`YVSIYVSTRTWVVHAGSNT`. Variant sequences are **built**, never read from a FASTA — each row's
residues are substituted into the wild type — so every variant is full length and every position is
checked against the sequence you supplied. A position that does not exist, or a residue code that
is not an amino acid, is refused at load time.

## The one question

Which backbone. Everything after follows from it, including what must be prepared first: a SaProt
backbone needs a wild-type 3Di string, which the panel builds in the same session from a structure
**you** supply — an AlphaFold `.cif`, your own `.pdb`, or ESMFold on the runtime. Nothing of the
kind is bundled, so that step always starts from your file; an ESM2 backbone needs nothing prepared
and the step is not on the page at all. How the model reads a sequence is not a question: it takes
...
on the seqdisplay-opt study's 16,424-variant library they were measured against; the panel
rescales them for your library — about a minute per run for the 124-variant bundled example — and
reports measured elapsed time afterwards. *load checked*: checkpoint downloaded and loaded, one pooled forward pass run, the
pooled vector asserted to be exactly the width the registry promises, LoRA injection validated
against the real module tree — not fine-tuned. *fine-tuned*: trained end to end here; see
[Status](#status) for what that run produced.

**Which to pick.** On a free T4, `ESM2-35M` (sequence only) or `SaProt-35M` (needs a 3Di string).
Run `ESM2-8M` first as a sanity check on a new library before paying for a long run.
...
`examples/mg8_petases/`: 124 MG8 PETase variants with one measured activity — `ln(1 + raw)` of the
value assayed relative to the wild type — at 19 mutated positions (22, 42, 49, 72, 78, 79, 80, 130,
131, 132, 150, 198, 199, 205, 206, 215, 216, 225, 279), and the 287-residue wild-type amino-acid
sequence. `examples/mg8_petases/README.md` documents each file, its schema, its derivation from the
source spreadsheet kept beside it, and the two pairs of rows that are the same protein twice. No
3Di string is bundled, and the library is small on purpose: a complete run fits inside one Colab
session, and the 13 test variants its 8:1:1 split leaves are a demonstration, not a benchmark.

The tuned hyperparameters in `config/best/` and every benchmark ρ in this README derive from the
re-evaluation tables of the seqdisplay-opt study (see [Acknowledgement](#acknowledgement)), which
measured them on a 16,424-variant SlugCas9 5NNK library; **that library is not redistributed
...
checkpoints, the prepared 3Di string, the three report files, both archives and
...
> **Data availability.** The example variant library analysed by ColabSeqDisplay — 124 MG8 PETase
...
The method — LoRA fine-tuning of protein language models on sequence-display libraries, the pooled
read-out, the multi-condition head, the evaluation protocol — and the tuned hyperparameters in
...
sets out what was taken, what was changed and what was not. Every benchmark number on this page is
that study's measurement, not ours.
