...
MG8 PETase example end to end; **Part 2** points the same workflow at your own protein;
...
trained — draws the report and writes both exports; the train button and ESMFold each refuse up
front.
...
124 MG8 PETase variants of a 287-residue protein, 19 mutated positions, one measured activity. The
library and the wild-type sequence ship with the repository, so with the defaults there is nothing
to upload and nothing to prepare before training. Leave every field alone: the bundled example,
`ESM2-35M`, `float32`, one split seed × one model seed, Drive off, `colabsd_work`, run name `run`.
...
> **124 variants** from `.../library.csv`.
> - Wild type: 287 residues.
> - 19 mutated sites at [22, 42, 49, 72, 78, 79, 80, 130, 131, 132, 150, 198, 199, 205, 206, 215,
>   216, 225, 279], wild-type residues `YVSIYVSTRTWVVHAGSNT`.
> - 1 conditions: activity.
...
loaded. `wild-type residues YVSIYVSTRTWVVHAGSNT` is the strongest check available: those are the
residues your wild type carries at the positions you listed, so an off-by-one or a signal peptide
in the FASTA shows up here and nowhere else. Nothing below runs until this step succeeds.

### Step 2 — the backbone, the only thing you are asked

The dropdown lists seven backbones cheapest-first, each naming whether it needs a 3Di string and
its estimated minutes per run. That is the whole question: how the model reads a sequence is fixed
— the embeddings at your mutated sites, averaged — so there is nothing else to choose and nothing
to compute before training. Leave the default for Part 1.

**A SaProt backbone adds one step**, a wild-type 3Di string, numbered 3 and pushing everything
below it down by one. Nothing of the kind is bundled, so it always starts from a structure you
supply. One thing the panel does not tell you: **a 3Di string from AlphaFold beats one from
ESMFold.** Download the model of your protein from <https://alphafold.ebi.ac.uk> and upload the
`.cif`: free, fast, and it matches your sequence exactly. ESMFold is the last resort, and its
free-T4 ceiling is 700 residues.
...
combinations* (residues drawn at random per mutated site, then de-duplicated — with the example's
19 sites a draw is effectively never a repeat), or your own CSV. Output is one row per variant with
the mutation columns, one predicted column per condition, and two summaries: **`pred_mean`**, and
**`pred_min`**, the worst condition — **rank by this when a hit has to work in every condition**.
...
Same notebook, same order; the differences are all in steps 1 and 2. You need `library.csv` (schema
and a worked row: [README](README.md#your-own-library)) and the wild-type sequence, pasted or as the
URL of a FASTA. A SaProt backbone also needs a structure of your wild type — a `.pdb` or `.cif`.
Nothing else is prepared for a new protein: the residues the model averages are the sites your
library mutates, which the CSV already names.
...
**What the backbone will cost you.** An ESM2 backbone: nothing, and no step 3 at all. A SaProt
backbone: seconds for a 3Di string built from a structure you downloaded, or an ESMFold run on the
runtime if you have no structure and the protein is inside its 700-residue free-T4 ceiling. A 3Di
string built in a session is offered back for the rest of it.

### Reading the PROVISIONAL warning

If the backbone you chose has never been tuned, the workflow says so in capitals
wherever it appears, and never stops you: the step 3 banner and its message board, the line printed
...
what it requires of your variants: the backbone, the mutated sites whose embeddings it averages,
the hyperparameters with `PROVISIONAL` in red if that is what they were, the unlock count in red if
it is zero, the conditions, and the variant columns with the positions they map to.
**Read the description line as the model's provenance** —
...
| ``Your variants table has no `p22` column(s). This bundle was trained on [...]`` | Your CSV column names do not match the bundle's. Rename them, or use the library those names came from. |
| `The header of <file> names ['p22'] more than once` | pandas would rename the second copy to `p22.1` and quietly ignore it, so the file is refused. Fix the header. A column reported missing next to a sibling like `p22.1` is the same problem reaching a later step. |
| `Unknown residue 'X' in column 'p22', row 0` | A residue outside the 20 standard ones, or three-letter data read as one-letter. The message says which way to flip the *Asn, not N* box. |
| `N condition values are missing or not numeric, for example column 'activity' at row 12` | Drop or fill those rows before loading. A sibling message catches values too large for float32 and suggests rescaling. |
...
| `<file> is a schema-1 bundle` (Predict) | It was exported by an older `colabsd`, whose model could be read out over a different set of residues; this one cannot tell which set it was, so it refuses rather than scoring on the wrong ones. Re-export it from its training run, or score it with the version the message names. |
...
and your library. A prepared 3Di string is worth keeping only if you plan to train the same protein
again in a fresh runtime — tick **Save to Google Drive** before it is written.
